...
`--llm` also prints terms not covered by the current taxonomy/aliases. That coverage-gap count is broader than the pending queue: pending rows are additionally filtered by threshold and candidate status (`pending` vs `approved`/`rejected`).
